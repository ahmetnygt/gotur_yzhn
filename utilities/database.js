...
const connectionLastUsed = {}; // Bağlantıların son kullanım zamanını tutar
...
function buildConnectionOptions() {
  const options = {
    ...
    logging: false,
    pool: {
      max: 5, // Her kiracı için max 5 bağlantı havuzu
      min: 0,
      acquire: 30000,
      idle: 10000
    }
...
// ARKA PLAN GÖREVİ: Memory Leak ve DB Connection Şişmesini Önleme
// Her 1 saatte bir çalışır, son 2 saattir GİÇBİR İŞLEM YAPMAYAN tenant'ların DB bağlantısını kapatır.
// Yeni bir istek geldiğinde sistem saliseler içinde otomatik olarak tekrar bağlanacaktır.
setInterval(() => {
  const now = Date.now();
  const IDLE_TIMEOUT = 2 * 60 * 60 * 1000; // 2 saat (milisaniye)

  for (const tenantKey of Object.keys(connections)) {
    if (now - connectionLastUsed[tenantKey] > IDLE_TIMEOUT) {
      console.log(`[${tenantKey}] veritabanı bağlantısı uzun süredir boşta olduğu için uykuya alındı.`);

      // MySQL bağlantı havuzunu temizle
      connections[tenantKey].sequelize.close().catch(err => console.error("DB Kapatma Hatası:", err));

      // RAM'den temizle
      delete connections[tenantKey];
      delete connectionLastUsed[tenantKey];
    }
  }
}, 60 * 60 * 1000).unref();
...
async function getTenantConnection(subdomain) {
  ...
  // (Böylece aktif firmalar asla uykuya geçmez)
  if (connections[tenantKey]) {
    connectionLastUsed[tenantKey] = Date.now();
    return connections[tenantKey];
  }

  // 2. ADIM: Race Condition Koruması. Bu tenant için şu an kurulum yapılıyorsa, diğer istekler beklesin.
  if (connectionPromises[tenantKey]) {
    return connectionPromises[tenantKey];
  }
  ...
  connectionPromises[tenantKey] = (async () => {
    try {
      ...
      // Şimdilik tabloları otomatik güncellemeye izin verdik.
      await sequelize.sync({ alter: true });

      // ---- SEED VERİLERİ (MEVCUT MANTIK AYNEN KORUNDU) ----
      ...
      // Kurulum bitti, bu tenantın işlemlerini hafızadan kaldır
      delete connectionPromises[tenantKey];
      connectionLastUsed[tenantKey] = Date.now();

      return connections[tenantKey];

    } catch (error) {
      // Hata olursa memory leak kalmaması için kilitleri temizle
      delete connectionPromises[tenantKey];
      throw error;
    }
  })();
